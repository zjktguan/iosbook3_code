...
- (void)viewDidLoad
{
    ...
    //从服务器获得产品标识列表
    NSString *strURL = [[NSString alloc] initWithFormat:@"http://iosbook1.com/service/ProductInfo.php"];
	NSURL *url = [NSURL URLWithString:strURL];
    
    __weak ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    [request setPostValue:@"eorient@163.com" forKey:@"email"];
    ...
    [request setCompletionBlock:^{
        ...
        NSNumber *rc = [resDict objectForKey:@"ResultCode"];
        
        if ([rc integerValue] == 0) {//成功
            NSLog(@"服务器查询产品信息成功。");
            NSArray* resArray = [resDict objectForKey:@"Products"];
            
            NSMutableArray* productIdArray = [NSMutableArray new];
            for (NSDictionary* product in resArray) {
                [productIdArray addObject:[product objectForKey:@"id"]];
            }
            //从NSArray转化为NSSet
            self.productIdentifiers = [[NSSet alloc] initWithArray:productIdArray];
            
        } else {
            NSLog(@"服务器查询产品信息失败。");
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"错误信息"
                                                                message:@"用户不存在，请到http://iosbook1.com注册。"
                                                               delegate:nil
...
- (void)completeTransaction:(SKPaymentTransaction *)transaction {
    ...
    NSString *base64JSONString = [transaction.transactionReceipt base64forData];
    
    NSString *strURL = [NSString stringWithFormat:@"http://iosbook1.com/service/verifyReceipt.php"];
    NSURL *url = [NSURL URLWithString:strURL];
    
    ASIFormDataRequest *request = [ASIFormDataRequest requestWithURL:url];
    [request setPostValue:base64JSONString forKey:@"receipt"];
    [request setPostValue:@"true" forKey:@"sandbox"];
    [request setPostValue:@"eorient@163.com" forKey:@"email"];
